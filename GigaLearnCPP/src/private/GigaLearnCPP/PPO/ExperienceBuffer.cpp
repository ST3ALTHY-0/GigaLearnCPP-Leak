...
GGL::ExperienceBuffer::ExperienceBuffer(int seed, torch::Device device) : seed(seed), device(device), rng(seed)
{
}
...
GGL::ExperienceTensors GGL::ExperienceBuffer::_GetSamples(const int64_t *indices, size_t size) const
{
	...
	auto tIndices = torch::from_blob((void *)indices,
									 {static_cast<long>(size)}, options)
						.clone();
